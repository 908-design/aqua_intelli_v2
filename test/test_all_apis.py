...
async def check(name: str, coro):
    try:
        ...
        RESULTS[name] = ("OK", status, detail)
    except Exception as e:
        RESULTS[name] = ("FAIL", "ERROR", str(e)[:80])
...
async def main():
    ...
    print("+----------------------------------------------------------+")
    print(f"  Test Location: {LAT} deg N, {LON} deg E (Hyderabad, India)")
    print()
    ...
    for name, _ in tests:
        ...
        s_icon = icon.encode('ascii', 'replace').decode()
        s_detail = detail.encode('ascii', 'replace').decode()
        print(f"  {s_icon} {name}  {status:<12} {s_detail}")

    print("  " + "-" * 90)
    print(f"\n  [OK] {ok} working   [WARN] {warn} need keys   [FAIL] {fail} errors\n")
