...
const MODULES = {
  ...
  city_drainage: {
    tag:'09 · CITY DRAINAGE', mode:'URBAN DRAINAGE & EQUITY MONITORING',
    lat:17.385, lon:78.487, name:'Hyderabad, Andhra Pradesh, India', zoom:11, color:'#39ff14',
    metrics:[
...
function switchModule(mod, tabEl) {
  ...
  // Clear specific layers upon module switch
  if (mod !== 'city_drainage' && typeof drainageLayerGroup !== 'undefined' && drainageLayerGroup && centerMap) {
...
function addVizagDrainageOverlays(lat, lon) {
  ...
  VIZAG_FACILITIES.pumps.forEach(f => {
    ...
    L.marker([f.lat, f.lon], {icon: L.divIcon({html, className: '', iconSize: [12, 12]})})
      .bindPopup(`<div style="font-family:'Share Tech Mono';">PUMP STATION: ${f.name}<br>CAP: ${f.cap}<br>STATUS: ${f.status.toUpperCase()}</div>`)
      .addTo(drainageLayerGroup);
  });
  ...
  VIZAG_FACILITIES.stps.forEach(f => {
    L.circleMarker([f.lat, f.lon], {radius: 7, color: '#ff5722', weight: 2, fillColor: '#ff5722', fillOpacity: 0.8})
      .bindPopup(`<div style="font-family:'Share Tech Mono';">STP PLANT: ${f.name}<br>STATUS: ${f.status.toUpperCase()}</div>`)
      .addTo(drainageLayerGroup);
  });
...
function setDrainView(type) {
  ...
  if (type === 'satellite') {
    ...
    satelliteLayer.addTo(centerMap);
    mapEl.classList.add('satellite-mode');
    btnSat.classList.add('active');
    btnMap.classList.remove('active');
  } else {
    if (satelliteLayer) centerMap.removeLayer(satelliteLayer);
...
function close360View() {
  document.getElementById('view360-overlay').classList.remove('show'); // Fallback for some browsers
...
function init360Scene() {
  ...
  const texture = new THREE.TextureLoader().load('https://upload.wikimedia.org/wikipedia/commons/thumb/6/6c/Aerial_view_of_Visakhapatnam_city.jpg/1280px-Aerial_view_of_Visakhapatnam_city.jpg');
  ...
  let isUserInteracting = false, onPointerDownPointerX = 0, onPointerDownPointerY = 0, onPointerDownLon = 0, onPointerDownLat = 0, lon = 0, lat = 0, phi = 0, theta = 0;

  container.addEventListener('pointerdown', e => {
    isUserInteracting = true;
    onPointerDownPointerX = e.clientX; onPointerDownPointerY = e.clientY;
  ...
  function animate() {
    ...
    v360Camera.target = new THREE.Vector3(500 * Math.sin(phi) * Math.cos(theta), 500 * Math.cos(phi), 500 * Math.sin(phi) * Math.sin(theta));
    v360Camera.lookAt(v360Camera.target);
    v360Renderer.render(v360Scene, v360Camera);
    document.getElementById('v360-bearing').textContent = `BEARING: ${Math.floor(lon % 360)}° &nbsp;|&nbsp; PIPE DEPTH: 2.4M &nbsp;|&nbsp; DIA: 600mm`;
  }
  animate();
}

function rotate360(deg) { /* logic to adjust 'lon' externally could be added */ }
function cycle360Site() { setStatus('SCANNING NEXT ACCESS POINT...', 'busy'); setTimeout(()=>setStatus('OPERATIONAL',''), 1500); }
function toggleDrainFlow() { flowAnimActive = !flowAnimActive; document.getElementById('dcb-flow').classList.toggle('active', flowAnimActive); }
function toggleDrainHeatmap() { heatmapActive = !heatmapActive; document.getElementById('dcb-heatmap').classList.toggle('active', heatmapActive); }
